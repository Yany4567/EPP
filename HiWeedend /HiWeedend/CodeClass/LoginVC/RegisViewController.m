...
-(NSString*)saveIDAction
{
    
    NSString*string=nil;
    KeychainItemWrapper*wrapper =[[KeychainItemWrapper alloc]initWithIdentifier:@"UDID" accessGroup:nil];
    NSString*udidStr=[wrapper objectForKey:(id)kSecValueData];
    
    if(udidStr.length ==0)
    {
        NSString*myUDIDStr=[[[UIDevice currentDevice]identifierForVendor]UUIDString];
        [wrapper setObject:myUDIDStr forKey:(id)kSecValueData];
        
    }else
    {
        
        string = [wrapper objectForKey:(id)kSecValueData];
        
        
    }
    NSLog(@"app的数据统计%@",string);
